...
export function collectObjectPropertyNames(
    ...
    return matcherObjectExpression.properties.reduce<ObjectPropertyCollection>(
        function (currentCollection, property) {
...
export function getMissingRequiredPropertyNames(
    ...
    return requiredPropertyNames.filter(function (requiredPropertyName) {
...
export function hasAnyRequiredProperty(
    ...
    return allowedPropertyNames.some(function (allowedPropertyName) {
...
export function resolveMatcherClassification(
    ...
    while (pendingResolutionSteps.length > 0) {
        ...
        const matcherClassification = currentStep.stepKind === 'expression'
...
function getDefinitionResolutionSteps(
    ...
    return variable.defs.map(function (definition) {
        return {
            definition,
...
function getDirectMatcherClassification(expression: Readonly<TSESTree.Expression>): MatcherClassification | undefined {
    const objectMatcherClassification = getObjectMatcherClassification(expression);
    if (objectMatcherClassification !== undefined) {
        return objectMatcherClassification;
    }
    const validationFunctionMatcherClassification = getValidationFunctionMatcherClassification(expression);
    if (validationFunctionMatcherClassification !== undefined) {
        return validationFunctionMatcherClassification;
    }
    const constructorMatcherClassification = getConstructorMatcherClassification(expression);
    if (constructorMatcherClassification !== undefined) {
        return constructorMatcherClassification;
    }
    return getRegexMatcherClassification(expression);
}
