...
const definitelySideEffectFreeExpressionTypes: ReadonlySet<string> = new Set([
...
function isDefinitelySideEffectFreeExpression(expression: Readonly<TSESTree.Expression>): boolean {
    return definitelySideEffectFreeExpressionTypes.has(expression.type);
}
...
function buildReplacementFix(
    ...
    return (fixer) => {
        ...
        const trailingArguments = assertionNode
            .arguments
            .slice(replacementPlan.consumedArgumentCount)
            .map(function (argumentNode) {
