...
const supportedBinaryOperators: ReadonlySet<string> = new Set([ '==', '!=', '===', '!==' ]);
...
function isSupportedBinaryOperator(
    ...
    return supportedBinaryOperators.has(operator);
}
...
function buildFix(
    ...
    const preservedArguments = assertionNode.arguments.slice(comparisonPlan.consumedArgumentCount).map(
        function (argument) {
