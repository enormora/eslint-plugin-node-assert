...
ruleTester.run('no-async-function-in-sync-assertion', noAsyncFunctionInSyncAssertionRule, {
    ...
    invalid: [
        ...
        {
            code:
                "import assert from 'node:assert/strict'; assert.doesNotThrow(async function () { await doThing(); });",
            errors: [
                {
                    messageId: 'no-async-function-in-sync-assertion',
        ...
        {
            code:
                "import { throws } from 'node:assert/strict'; const callback = async () => { await doThing(); }; throws(callback);",
            errors: [
                {
                    messageId: 'no-async-function-in-sync-assertion',
                    data: { methodName: 'throws', asyncMethodName: 'rejects' }
        ...
        {
            code:
                "import assert from 'node:assert'; const { strict } = assert; async function callback() { await doThing(); } strict.doesNotThrow(callback);",
            errors: [
                {
                    messageId: 'no-async-function-in-sync-assertion',
                    data: { methodName: 'doesNotThrow', asyncMethodName: 'doesNotReject' }
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const originalCallback = async () => { await doThing(); }; const aliasedCallback = originalCallback; assert.throws(aliasedCallback);",
            errors: [
                {
                    messageId: 'no-async-function-in-sync-assertion',
                    data: { methodName: 'throws', asyncMethodName: 'rejects' }
