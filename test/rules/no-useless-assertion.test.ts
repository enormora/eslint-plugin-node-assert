...
ruleTester.run('no-useless-assertion', noUselessAssertionRule, {
    ...
    invalid: [
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const { ifError: failIfError } = assert; failIfError(null);",
            errors: [ { messageId: 'always-passes' } ]
        },
        ...
        {
            code:
                "import { deepStrictEqual } from 'node:assert/strict'; const eq = deepStrictEqual; eq({ a: 1 }, { a: 2 });",
            errors: [ { messageId: 'always-fails' } ]
        },
        {
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const key = 'partialDeepStrictEqual'; assert[key]({ a: 1 }, { a: 2 });",
            errors: [ { messageId: 'always-fails' } ]
        },
        ...
        {
            code:
                "import assert from 'node:assert/strict'; assert.ok(true); assert.strictEqual(1, 1); assert.ifError(null);",
