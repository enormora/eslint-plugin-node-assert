...
ruleTester.run('prefer-deep-equality', preferDeepEqualityRule, {
    ...
    invalid: [
        ...
        {
            code: "import assert, { strictEqual } from 'node:assert'; assert.strictEqual(a, { ok: true }); " +
                'strictEqual(b, [1, 2]);',
            errors: [ { messageId: 'prefer-deep-equality' }, { messageId: 'prefer-deep-equality' } ],
            output: "import assert, { strictEqual } from 'node:assert'; assert.deepStrictEqual(a, { ok: true }); " +
                'strictEqual(b, [1, 2]);'
        },
        ...
        {
            code:
                "import assert from 'node:assert'; const { strict } = assert; strict.strictEqual(result, { ok: true });",
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output:
                "import assert from 'node:assert'; const { strict } = assert; strict.deepStrictEqual(result, { ok: true });"
        },
        {
        ...
        {
            code: "import assert from 'node:assert'; const { strict } = assert; const s = strict; " +
                's.strictEqual(result, { ok: true });',
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output: "import assert from 'node:assert'; const { strict } = assert; const s = strict; " +
                's.deepStrictEqual(result, { ok: true });'
        },

        // const namespace alias
        {
            code: "import assert from 'node:assert/strict'; const alias = assert; " +
                'alias.strictEqual(result, { ok: true });',
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output: "import assert from 'node:assert/strict'; const alias = assert; " +
                'alias.deepStrictEqual(result, { ok: true });'
        },
        // Multi-hop alias chain
        {
            code: "import assert from 'node:assert/strict'; const a = assert; const b = a; " +
                'b.strictEqual(result, { ok: true });',
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output: "import assert from 'node:assert/strict'; const a = assert; const b = a; " +
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const { strictEqual } = assert; strictEqual(result, { ok: true });",
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output: null
        },
        {
            code:
                "import assert from 'node:assert/strict'; const { strictEqual: eq } = assert; eq(result, { ok: true });",
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output: null
        },
        {
        ...
        {
            code: "import { strictEqual } from 'node:assert/strict'; const eq1 = strictEqual; const eq2 = eq1; " +
                'eq2(result, { ok: true });',
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output: null
        },
        ...
        {
            code:
                "import assert from 'node:assert/strict'; function test() { assert.strictEqual(result, { ok: true }); }",
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output:
                "import assert from 'node:assert/strict'; function test() { assert.deepStrictEqual(result, { ok: true }); }"
        },
        {
        ...
        },
        {
            ...
            errors: [ { messageId: 'prefer-deep-equality' } ],
            output:
                "import assert from 'node:assert/strict'; class C { static { assert.deepStrictEqual(result, {}); } }"
        },
        ...
        {
            code: "import assert from 'node:assert/strict'; assert.strictEqual(a, { ok: true }); " +
                'assert.equal(b, [1, 2]);',
            errors: [ { messageId: 'prefer-deep-equality' }, { messageId: 'prefer-deep-equality' } ],
            output: "import assert from 'node:assert/strict'; assert.deepStrictEqual(a, { ok: true }); " +
