...
ruleTester.run('prefer-partial-deep-strict-equal', preferPartialDeepStrictEqualRule, {
    ...
    invalid: [
        ...
        {
            code:
                "import assert from 'node:assert/strict'; assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); assert.strictEqual(user.email, 'a@b');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Mixed eligible methods on the same root
        {
            code:
                "import assert from 'node:assert/strict'; assert.strictEqual(user.id, 1); assert.deepStrictEqual(user.profile, { name: 'Alice' });",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Two deepStrictEqual calls on the same root
        {
            code:
                "import assert from 'node:assert/strict'; assert.deepStrictEqual(user.profile, { name: 'Alice' }); assert.deepStrictEqual(user.address, { city: 'X' });",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Same property path twice still constitutes a run on the same root
        {
            code:
                "import assert from 'node:assert/strict'; assert.strictEqual(user.id, 1); assert.strictEqual(user.id, 2);",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Namespace import binding
        {
            code:
                "import * as assert from 'node:assert/strict'; assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Named import (no alias)
        {
            code:
                "import { strictEqual } from 'node:assert/strict'; strictEqual(user.id, 1); strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        ...
        {
            code:
                "import assert, { deepStrictEqual } from 'node:assert/strict'; assert.strictEqual(user.id, 1); deepStrictEqual(user.profile, {});",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Base specifier (`node:assert`)
        {
            code:
                "import assert from 'node:assert'; assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Bare specifiers without the node: protocol
        {
            code:
                "import assert from 'assert/strict'; assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        {
            code:
                "import assert from 'assert'; assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        {
            code:
                "import { strictEqual } from 'assert/strict'; strictEqual(user.id, 1); strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // strict re-export through a named import
        {
            code:
                "import { strict } from 'node:assert'; strict.strictEqual(user.id, 1); strict.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // strict re-export through a renamed named import
        {
            code:
                "import { strict as s } from 'node:assert'; s.strictEqual(user.id, 1); s.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // strict re-export through const destructuring
        {
            code:
                "import assert from 'node:assert'; const { strict } = assert; strict.strictEqual(user.id, 1); strict.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // strict re-export through renamed const destructuring
        {
            code:
                "import assert from 'node:assert'; const { strict: s } = assert; s.strictEqual(user.id, 1); s.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // const namespace alias
        {
            code:
                "import assert from 'node:assert/strict'; const a = assert; a.strictEqual(user.id, 1); a.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Multi-hop alias
        {
            code:
                "import assert from 'node:assert/strict'; const a = assert; const b = a; b.strictEqual(user.id, 1); b.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Destructured method
        {
            code:
                "import assert from 'node:assert/strict'; const { strictEqual } = assert; strictEqual(user.id, 1); strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Renamed destructured method
        {
            code:
                "import assert from 'node:assert/strict'; const { strictEqual: eq } = assert; eq(user.id, 1); eq(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Re-binding a named import
        {
            code:
                "import { strictEqual } from 'node:assert/strict'; const eq = strictEqual; eq(user.id, 1); eq(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const key = 'strictEqual'; assert[key](user.id, 1); assert[key](user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Mixed bindings resolving to the same method on the same root
        {
            code:
                "import assert from 'node:assert/strict'; const a = assert; const b = assert; a.strictEqual(user.id, 1); b.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Two separate runs in the same file produce two reports
        {
            code:
                "import assert from 'node:assert/strict'; assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); const sep = 1; assert.strictEqual(account.balance, 100); assert.strictEqual(account.owner, 'Bob');",
            errors: [
        ...
        {
            code:
                "import assert from 'node:assert/strict'; assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); assert.strictEqual(account.balance, 100); assert.strictEqual(account.owner, 'Bob');",
            errors: [
                { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } },
                { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'account' } }
            ]
        ...
        {
            code:
                "import assert from 'node:assert/strict'; assert.strictEqual(user.id, 1); assert.strictEqual(account.balance, 100); assert.strictEqual(account.owner, 'Bob');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'account' } } ]
        },
        // Inside a function body
        {
            code:
                "import assert from 'node:assert/strict'; function test() { assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); }",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Inside an arrow function body
        {
            code:
                "import assert from 'node:assert/strict'; const test = () => { assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); };",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Inside an `if` block
        {
            code:
                "import assert from 'node:assert/strict'; if (cond) { assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); }",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Inside a try block
        {
            code:
                "import assert from 'node:assert/strict'; try { assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); } catch (error) {}",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Inside a switch case consequent
        {
            code:
                "import assert from 'node:assert/strict'; switch (x) { case 1: assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); break; }",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Inside a class static block
        {
            code:
                "import assert from 'node:assert/strict'; class C { static { assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); } }",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Run preceded by a non-eligible call still triggers
        {
            code:
                "import assert from 'node:assert/strict'; console.log('start'); assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice');",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Run with deeper nested property paths (root identifier shared)
        {
            code:
                "import assert from 'node:assert/strict'; assert.strictEqual(user.profile.name, 'Alice'); assert.strictEqual(user.id, 1); assert.deepStrictEqual(user.address, {});",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        },
        // Run inside a nested function inside a block
        {
            code:
                "import assert from 'node:assert/strict'; if (cond) { function inner() { assert.strictEqual(user.id, 1); assert.strictEqual(user.name, 'Alice'); } }",
            errors: [ { messageId: 'prefer-partial-deep-strict-equal', data: { rootName: 'user' } } ]
        }
    ]
});
