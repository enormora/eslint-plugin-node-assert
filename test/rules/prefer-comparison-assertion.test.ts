...
ruleTester.run('prefer-comparison-assertion', preferComparisonAssertionRule, {
    ...
    invalid: [
        ...
        },
        {
            ...
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
            output:
                "import assert from 'node:assert/strict'; const a = assert; const b = a; b.strictEqual(actual, expected);"
        },
        {
        ...
        },
        {
            ...
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
            output:
                "import assert from 'node:assert'; const { strict: s } = assert; s.notStrictEqual(actual, expected);"
        },
        ...
        {
            code:
                "import assert from 'node:assert/strict'; assert[`strictEqual`](42 === result.deep.value, true, message);",
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
        ...
        {
            code:
                "import assert from 'node:assert'; assert[`notStrictEqual`](42 === result.deep.value, false, message);",
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
            output: "import assert from 'node:assert'; assert[`strictEqual`](42, result.deep.value, message);"
        },
        {
            code:
                "import assert from 'node:assert'; const { strict: s } = assert; s.notStrictEqual(actual !== expected, false);",
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
            output:
                "import assert from 'node:assert'; const { strict: s } = assert; s.notStrictEqual(actual, expected);"
        },
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const { notStrictEqual: neq } = assert; const a = neq; a(actual === expected, false);",
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
            output: null
        },
        {
        ...
        {
            code:
                "import assert from 'node:assert'; const { equal: eq } = assert; const a = eq; a(actual != expected, true);",
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
            output: null
        },
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const { ok: okay } = assert; const a = okay; a(actual !== expected);",
            errors: [ { messageId: 'prefer-comparison-assertion' } ],
            output: null
        },
        ...
        {
            code:
                "import assert from 'node:assert'; assert.ok(a == b); assert.equal(c !== d, false); assert.strictEqual(e === f, true);",
            errors: [
            ...
            output:
                "import assert from 'node:assert'; assert.equal(a, b); assert.strictEqual(c, d); assert.strictEqual(e, f);"
        }
    ]
});
