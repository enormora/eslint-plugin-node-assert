...
ruleTester.run('require-error-matcher', requireErrorMatcherRule, {
    valid: [
        ...
        {
            code:
                "import assert from 'node:assert/strict'; await assert.rejects(promise, { name: 'TypeError', message: 'invalid input' });",
            options: [ { allowedMatchers: [ 'object' ], objectMatcher: { requiredProperties: [ 'name', 'message' ] } } ]
        },
        {
            code:
                "import assert from 'node:assert/strict'; await assert.rejects(promise, { code: 'ERR_INVALID_INPUT' });",
            options: [
                {
        ...
        {
            code:
                "import { strict as strictAssert } from 'node:assert'; await strictAssert.rejects(promise, /invalid input/);"
        },
        {
            code:
                "import assert from 'node:assert/strict'; const strictAssert = assert; strictAssert.throws(fn, TypeError);"
        }
    ],
    invalid: [
        {
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const expectedErrorMatcher = { code: 'ERR_INVALID_INPUT' }; await assert.rejects(promise, expectedErrorMatcher);",
        ...
        {
            code:
                "import assert from 'node:assert/strict'; await assert.rejects(promise, { message: 'invalid input' });",
            options: [ {
                allowedMatchers: [ 'object' ],
                objectMatcher: { requiredProperties: [ 'name', 'message' ] }
            } ],
            errors: [
                {
                    messageId: 'missing-required-object-properties',
