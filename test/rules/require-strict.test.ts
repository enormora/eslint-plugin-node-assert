...
ruleTester.run('require-strict', requireStrictRule, {
    valid: [
        ...
        {
            code:
                "import assert from 'node:assert/strict'; const { strictEqual } = assert; strictEqual(actual, expected);",
            options: [ { mode: 'explicit' } ]
        },
    ...
    invalid: [
        ...
        },
        {
            ...
            errors: [ { messageId: 'require-strict' } ],
            output:
                "import assert from 'node:assert'; const alias = assert; alias['deepStrictEqual'](actual, expected);"
        },
        ...
        {
            code:
                "import assert from 'node:assert'; const { equal: strictEqual } = assert; strictEqual(actual, expected);",
            errors: [ { messageId: 'require-strict' } ],
            output: null
        },
